...
- **Explore the codebase** before asking about anything that could be answered by reading the code. Delegate this to the built-in `Explore` agent rather than doing it inline in the main thread. Do not ask the human what already exists — find it yourself.
