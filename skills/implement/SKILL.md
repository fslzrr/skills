...
3. Map each acceptance criterion to one SUBTASK. Each SUBTASK is one RED/GREEN/REFACTOR cycle.
4. Present the SUBTASK plan to the human:
...
5. Wait for explicit human approval. Apply any adjustments.

### Execute (repeat for each SUBTASK)

6. **Follow the `/tdd` procedure** for this SUBTASK's behavior.
...
7. **Follow the `/review` procedure** on the changes made during this SUBTASK.

   - If verdict is **FAIL** (blocking findings exist):
     - Record the feedback
     - Return to step 6 (`/tdd` step c — implementation) with the blocking findings as explicit constraints
...
9. **Run all new or modified tests** and confirm everything is GREEN before starting the next SUBTASK.

### Full suite check

10. After all SUBTASKs are committed, run the **full test suite**.
    - If it fails on something **within scope** of this TASK: treat it as a new SUBTASK. Return to step 6.
...
11. Present the **advisory log** accumulated across all SUBTASKs:
...
12. Ask: "Implementation is complete and all tests are GREEN. Shall I open the PR?"
...
14. Use `/issues` to transition the TASK from `ai-in-progress` → `in-code-review`.

15. **Remain active** in this conversation. The human may give PR feedback or request changes directly here. When they do, implement the requested changes and push to the same branch. The TASK stays `in-code-review` until the human merges the PR (GitHub auto-closes the TASK on merge).
